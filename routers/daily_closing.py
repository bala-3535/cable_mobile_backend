...
def get_daily_summary(
    ...
    existing_report = db.query(DailyClosing).filter(
        ...
        DailyClosing.tenant_id == current_user.tenant_id
    ).first()
...
def list_daily_closings(
    ...
    ).join(User, DailyClosing.agent_id == User.id).filter(DailyClosing.tenant_id == current_user.tenant_id)
...
def verify_daily_closing(
    ...
    report = db.query(DailyClosing).filter(DailyClosing.id == report_id, DailyClosing.tenant_id == current_user.tenant_id).first()
    if not report:
    ...
    report.verified_at = get_ist_now()
