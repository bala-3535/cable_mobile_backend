...
def ensure_billing_record(db: Session, customer: Customer) -> BillingRecord:
    ...
    # Get package price
    db_package = db.query(Package).filter(Package.name == customer.subscription_plan).first()
...
def create_customer(
    ...
    billing_day = customer_in.billing_day if hasattr(customer_in, 'billing_day') and customer_in.billing_day else datetime.now().day
    billing_period = datetime.now().strftime("%Y-%m")
...
def get_ai_overview(
    ...
    recent_transactions = db.query(Transaction, Customer.customer_name)\
        .join(Customer).order_by(Transaction.payment_date.desc()).limit(15).all()
...
def get_dashboard_summary(
    ...
    today_collection = db.query(func.sum(Transaction.amount))\
        .filter(Transaction.payment_date >= today_start).scalar() or 0
        
    # New customers today
    new_customers = db.query(Customer).filter(Customer.created_at >= today_start).count()
...
def get_customer(
    ...
    history = db.query(BillingRecord).filter(BillingRecord.customer_id == customer_id).order_by(BillingRecord.billing_period.desc()).all()
...
def get_billing_history(
    ...
    return db.query(BillingRecord).filter(BillingRecord.customer_id == customer_id).order_by(BillingRecord.billing_period.desc()).all()
...
async def upload_customers(
    ...
    for row in mapped_rows:
        try:
            ...
            if existing:
                errors.append(f"Customer {acc_num} already exists in database")
                continue
